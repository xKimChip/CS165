...
def first_fit2(items: list[float], assignment: list[int], free_space: list[float]):
    binTree = ZipZipTree(capacity = len(items))
    free_space.append(Decimal(str(1.0)))
    binTree.insert(0, Decimal(str(1.0)))
    ...
    for i in range(len(items)):
        ...
        if cur and Decimal(str(cur.value)) >= Decimal(str(items[i])):
            j += 1
            cur = binTree.insert(j, Decimal(str(1.0)) - Decimal(str(items[i])))
            keyFind = cur.key
            assignment[i] = j
            free_space.append(Decimal(str(1.0)) - Decimal(str(items[i])))
        else:
            ...
            keyFind = cur.key
            free_space[keyFind] -= Decimal(str(items[i]))
        ...
        if cur.value > free_space[cur.key]:
            # rebalance the tree
...
def first_fit(items: list[float], assignment: list[int], free_space: list[float]):
    ...
    for i in range(len(items)):
        ...
        if cur and Decimal(str(cur.value)) < Decimal(str(items[i])):
            ...
            keyFind = cur.key
            assignment[i] = j
            free_space.append(Decimal(str(1)) - Decimal(str(items[i])))
        else:
            # find the first bin according to freeSpace
